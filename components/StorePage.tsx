...
export default function StorePage({ products, categories, settings, paymentMethods }: Props) {
  ...
  const catIcons: Record<string, string> = {
    'تنسيقات ورد': '🌷', 'هدايا جاهزة': '🎁', 'عطور': '🧴',
    'نباتات داخلية': '🌿', 'مناسبات': '🎈', 'الكل': '🛍️'
  }
  ...
  return (
    <div className="store-outer" dir={lang === 'ar' ? 'rtl' : 'ltr'} style={{ fontFamily: 'Tajawal, sans-serif' }}>
      <div className="store-container" style={{ background: '#fff', position: 'relative' }}>
        ...
        {/* ===== الهيرو: صندوق صورة بارتفاع ثابت 220px مع object-fit: cover لتناسق الحجم بين كل الصور ===== */}
        <div className="hero" style={{
          ...
          <div style={{ flex: '1 1 160px', position: 'relative', zIndex: 2, minWidth: '150px' }}>
            <h1 style={{ color: '#3D1A24', fontSize: '20px', fontWeight: 700, margin: '0 0 4px', lineHeight: 1.3 }}>{currentSlide.title}</h1>
            <h2 style={{ color: '#E8638A', fontSize: '17px', fontWeight: 700, margin: '0 0 8px' }}>{currentSlide.subtitle}</h2>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px' }}>
              <div style={{ width: '28px', height: '1px', background: '#C8956C' }} />
              <span style={{ color: '#C8956C', fontSize: '12px' }}>✿</span>
              <div style={{ width: '28px', height: '1px', background: '#C8956C' }} />
            </div>
            <p style={{ color: '#999', fontSize: '12px', margin: '0 0 16px', lineHeight: 1.5 }}>{currentSlide.desc}</p>
            <button onClick={() => { setSelectedCategory('all'); scrollToProducts() }} style={{ background: '#E8638A', color: '#fff', border: 'none', borderRadius: '25px', padding: '10px 20px', fontSize: '13px', fontWeight: 600, cursor: 'pointer', fontFamily: 'Tajawal, sans-serif', boxShadow: '0 4px 15px rgba(232,99,138,0.3)' }}>{t.shopNow}</button>
          </div>
          ...
          <div style={{
            ...
            overflow: 'hidden',
            position: 'relative',
            boxShadow: '0 8px 24px rgba(61,26,36,0.12)',
          }}>
            {currentSlide.image_url ? (
              ...
              <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '60px', opacity: 0.2 }}>🌸</div>
            )}
          </div>

          <div style={{ width: '100%', display: 'flex', justifyContent: 'center', gap: '6px', zIndex: 2 }}>
            {heroSlides.map((_, i) => (
              <button key={i} onClick={() => setHeroSlide(i)} style={{ width: i === heroSlide ? '20px' : '8px', height: '8px', borderRadius: '4px', background: i === heroSlide ? '#E8638A' : '#ddd', border: 'none', cursor: 'pointer', transition: 'all 0.3s' }} />
            ))}
          </div>
        </div>
        {/* ===== نهاية قسم الهيرو ===== */}
        ...
        <div style={{ padding: '20px 16px', background: '#fff', borderBottom: '1px solid #F5E8EC' }}>
          <div className="categories-row" style={{ display: 'flex', gap: '16px', overflowX: 'auto', paddingBottom: '4px' }}>
            <div onClick={() => { setSelectedCategory('all'); scrollToProducts() }} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px', cursor: 'pointer', minWidth: '64px' }}>
              <div style={{ width: '60px', height: '60px', borderRadius: '50%', background: selectedCategory === 'all' ? '#FDE8EF' : '#F8F8F8', border: '2px solid ' + (selectedCategory === 'all' ? '#E8638A' : '#eee'), display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px', transition: 'all 0.2s' }}>🛍️</div>
            ...
            {categories.map(cat => (
              <div key={cat.id} onClick={() => { setSelectedCategory(cat.id); scrollToProducts() }} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px', cursor: 'pointer', minWidth: '64px' }}>
                <div style={{ width: '60px', height: '60px', borderRadius: '50%', background: selectedCategory === cat.id ? '#FDE8EF' : '#F8F8F8', border: '2px solid ' + (selectedCategory === cat.id ? '#E8638A' : '#eee'), display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px', transition: 'all 0.2s' }}>{catIcons[cat.name] || '🎀'}</div>
          ...
          {promoCards.map((card, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                <p style={{ color: '#3D1A24', fontWeight: 700, fontSize: '12px', margin: '0 0 10px', lineHeight: 1.4, position: 'relative', zIndex: 2 }}>{card.title}</p>
                <button onClick={() => { setSelectedCategory(targetCategory); scrollToProducts() }} style={{ background: btnColor, color: '#fff', border: 'none', borderRadius: '12px', padding: '5px 10px', fontSize: '11px', cursor: 'pointer', fontFamily: 'Tajawal, sans-serif', fontWeight: 600, position: 'relative', zIndex: 2 }}>{btnText}</button>
                {!card.image_url && card.emoji && (
                  <div style={{ fontSize: '32px', position: 'absolute', bottom: '-6px', left: '-4px', opacity: 0.2, pointerEvents: 'none', zIndex: 1 }}>{card.emoji}</div>
                )}
              </div>
            )
          })}
